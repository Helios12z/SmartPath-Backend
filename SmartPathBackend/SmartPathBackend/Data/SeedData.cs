...
namespace SmartPathBackend.Data
{
    public static class SeedData
    {
        ...
        public static async Task SeedAsync(SmartPathDbContext db, CancellationToken ct = default)
        {
            ...
            // ===== 5.5) POST SEARCH INDICES (for search engine) =====
            var postSearchIndices = BuildPostSearchIndices(posts, categories);
            await UpsertPostSearchIndicesAsync(db, postSearchIndices, ct);
        ...
        private static List<PostSearchIndex> BuildPostSearchIndices(List<Post> posts, List<Category> categories)
        {
            var searchIndices = new List<PostSearchIndex>();
            var random = new Random(42); // Fixed seed for reproducible data

            // Get all accepted posts for search indexing
            var acceptedPosts = posts.Where(p => p.Status == Status.Accepted).ToList();

            foreach (var post in acceptedPosts)
            {
                // Create mock category data for the post
                var postCategories = categories
                    .Where(c => random.Next(0, 3) > 0) // 66% chance to include each category
                    .Take(random.Next(1, 4)) // 1-3 categories per post
                    .ToList();

                // Generate some tags based on content
                var tags = new List<string>();
                var contentLower = (post.Title + " " + post.Content).ToLower();

                // Add some common tech terms as tags
                var techTags = new[] { "csharp", "javascript", "react", "api", "database", "programming",
                                     "tutorial", "guide", "help", "question", "answer", "development",
                                     "web", "frontend", "backend", "sql", "entity-framework", "dotnet" };

                foreach (var tag in techTags)
                {
                    if (contentLower.Contains(tag) && random.Next(0, 3) > 0)
                    {
                        tags.Add(tag);
                    }
                }

                // Add random tags if no content-based tags found
                if (tags.Count == 0)
                {
                    tags.AddRange(techTags.OrderBy(x => random.Next()).Take(random.Next(2, 5)));
                }

                var searchIndex = new PostSearchIndex
                {
                    Id = 0, // Auto-generated by database
                    PostId = post.Id,
                    Title = post.Title,
                    Content = post.Content,
                    Summary = post.Content.Length > 500 ? post.Content[..500] + "..." : post.Content,
                    IsQuestion = post.IsQuestion,
                    IsSolved = false, // Posts don't have IsSolved property yet
                    ViewCount = random.Next(0, 1000), // Mock view count
                    LikeCount = random.Next(0, 50), // Mock like count
                    CommentCount = random.Next(0, 20), // Mock comment count
                    CreatedAt = post.CreatedAt,
                    UpdatedAt = post.UpdatedAt ?? post.CreatedAt,
                    AuthorId = post.AuthorId,
                    AuthorName = $"User {post.AuthorId:N}", // Mock author name
                    AuthorUsername = $"user_{post.AuthorId:N}", // Mock username
                    AuthorAvatar = "", // Mock avatar
                    CategoryIds = System.Text.Json.JsonSerializer.Serialize(postCategories.Select(c => c.Id)),
                    CategoryNames = System.Text.Json.JsonSerializer.Serialize(postCategories.Select(c => c.Name)),
                    CategorySlugs = System.Text.Json.JsonSerializer.Serialize(postCategories.Select(c => c.Name.ToLower().Replace(" ", "-"))),
                    Tags = System.Text.Json.JsonSerializer.Serialize(tags.Distinct().Take(5)), // Limit to 5 tags
                    Embedding = null, // Will be generated when embedding service is available
        ...
        private static async Task UpsertPostSearchIndicesAsync(SmartPathDbContext db, IEnumerable<PostSearchIndex> searchIndices, CancellationToken ct)
        {
            var existing = await db.PostSearchIndices.AsNoTracking()
                .Select(si => si.PostId)
                .ToListAsync(ct);

            var existingPostIds = new HashSet<Guid>(existing);

            foreach (var searchIndex in searchIndices)
            {
                if (!existingPostIds.Contains(searchIndex.PostId))
                {
                    await db.PostSearchIndices.AddAsync(searchIndex, ct);
                }
            }
        }
    }
